...
def test_docToMongo_strategy_additionalProperties():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "additionalproperties" in reason_lower, f"Reason should mention additionalProperties: {storage_plan['entities'][0]['reason']}"
...
def test_docToMongo_strategy_object_no_properties():
    """Test that docToMongo strategy allows object with no properties/additionalProperties to remain postgres."""
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        workspace_root = temp_path / "test_workspace"
        workspace_root.mkdir()
        ...
        # Create contract with entity that has object field with no properties/additionalProperties
        contract = {
            "source_repo_url": "https://github.com/test/repo",
            "framework": {"name": "vite", "versionHint": "^6.0.0"},
            "envVars": [],
            ...
            "entities": [
                {
                    "name": "SimpleEntity",
                    "sourcePath": "src/Entities/SimpleEntity.json",
                    "fields": [
                        {"name": "id", "type": "string", "required": True, "nullable": False, "raw": {"type": "string"}},
                        {"name": "name", "type": "string", "required": True, "nullable": False, "raw": {"type": "string"}},
                        {
                            ...
                            "raw": {
                                "type": "object",
                                "description": "Simple object with no properties"
                            }
                        }
                    ],
                    "relationships": [],
        ...
        assert storage_plan["entities"][0]["store"] == "postgres", "Object with no properties/additionalProperties should remain postgres in docToMongo strategy"
